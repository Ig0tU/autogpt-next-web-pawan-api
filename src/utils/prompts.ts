...
import { GPT_35_TURBO, MODEL_PROVIDERS, DEFAULT_HF_MODEL } from "./constants";
import { HuggingFaceLLM } from "../llms/huggingface";
...
export const createModel = (settings: ModelSettings) => {
  if (settings.customProvider === MODEL_PROVIDERS.HUGGINGFACE) {
    const hfApiKey = settings.huggingFaceApiKey || process.env.HUGGINGFACE_API_KEY;
    if (!hfApiKey) {
    ...
    return new HuggingFaceLLM({
      apiKey: hfApiKey,
      modelName: settings.huggingFaceModelName || DEFAULT_HF_MODEL,
      temperature: settings.customTemperature || 0.9,
      maxTokens: settings.customMaxTokens || 400,
    });
  } else {
    // Default to OpenAI
